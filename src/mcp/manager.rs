...
impl Default for SessionManager {
    fn default() -> Self {
        // Try to detect the correct Zellij socket directory
        // Priority: runtime_dir/zellij/VERSION > runtime_dir/zellij > /tmp/zellij
        let socket_dir = if let Some(runtime_dir) = dirs::runtime_dir() {
            let base_dir = runtime_dir.join("zellij");

            // Try to find versioned directory (e.g., /run/user/1000/zellij/0.43.1)
            if base_dir.exists() {
                if let Ok(entries) = std::fs::read_dir(&base_dir) {
                    ...
                    if let Some(first_version) = version_dirs.first() {
                        tracing::debug!("Using Zellij socket dir: {:?}", first_version);
                        first_version.clone()
                    } else {
                        tracing::debug!("Using Zellij socket dir: {:?}", base_dir);
                        base_dir
                    }
                } else {
                    tracing::debug!("Using Zellij socket dir: {:?}", base_dir);
                ...
                tracing::debug!("Base dir doesn't exist, using: {:?}", base_dir);
                base_dir
            }
        } else {
